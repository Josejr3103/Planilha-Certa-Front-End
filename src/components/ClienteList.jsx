...
function ClienteList() {
  ...
  const confirmarEdicao = async () => {
    ...
    try {
      await ClienteService.editarCliente(editandoCliente.idCliente, clienteAtualizado);
            ...
            {clientes.map((cliente) => (
              <tr key={cliente.idCliente}>
                {editandoCliente?.idCliente === cliente.idCliente ? (
                  ...
                  <>
                    <td className='cliente-nome'>{cliente.nomeCliente}</td>
                    <td className='cliente-categoria'>{cliente.categoriaCliente}</td>
                    <td className='acoes'>
                    <button onClick={() => iniciarEdicao(cliente)} className="Btn">Editar <svg class="svg" viewBox="0 0 512 512">
